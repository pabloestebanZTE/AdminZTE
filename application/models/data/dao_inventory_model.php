  ...
  class dao_inventory_model extends CI_Model{
    ...
    public function insertEquipment($equipment, $pvd){
      ...
      if ($session != "false"){
        ...
        $row = $result->fetch_assoc();
        $row = $row['count(*)']+1;
        $sql2 = "INSERT INTO stuff (K_IDSTUFF, K_IDMODEL, N_SERIAL, N_PLACAINVENTARIO, N_PARTE, N_ESTADO, K_IDSTUFF_CATEGORY, K_IDPVD, Q_PROGRESS)
          values (".$row.", ".$equipment->getModelo().", '".$equipment->getSerial()."', '".$equipment->getPlaca()."', '".$equipment->getParte()."', '".$equipment->getEstado()."', ".$equipment->getCategoria().", ".$pvd.", ".$equipment->getProgress().");";
        $session->query($sql2);
        $respuesta = "ok";
      } else {
        $respuesta = "Error de informacion";
